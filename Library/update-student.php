	...
	window.addEventListener('load', function() { //window on load function
		...
		$(".action-button").click(function() { //table row action button click listner
			var $row = $(this).closest('tr')
				var $studentID = $row.find(".studentID").text(); //gets student ID from the clicked row

			document.location.href = "add-student.php?studentID=" + $studentID; // redirect to add-student form with student ID   	    
		});
...
if($_POST){  // checks if there is post request
	$searchValue = $_POST['searchValue']; // get searchValue from post request
	$searchBy = $_POST['searchBy'];	// get searchBy from post request

	$sql ="";

	if($searchBy == 'Last Name') { //if searchBy Last Name
		$sql ="SELECT * FROM  `users` WHERE last_name LIKE '%$searchValue%' and role = 'student'";
	} else if($searchBy == 'Student ID') { //else if searchBy Student ID
	...
	if ($result -> num_rows > 0) { //if result row gt than 0
		while($row = $result->fetch_assoc()) {
			...
			// add user_id, last_name, first_name, phone_number in html table
